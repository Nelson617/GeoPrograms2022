public class B4_MiltonFilter extends World {
   ...
   public void go() {
      plane.loadBackGround("tom.jpeg");          //*** new method to load a diffrent JPG
      System.out.println("This will be printed to the  window. ");
      plane.teleport(0, 0);
      plane.showBackGround();
      System.out.println(plane.howMuchRed());

      for (int row = 30; row < 120; row++)    //*** this is using new getBackground
      {
         for (int col = 300; col < 490; col++)    //*** this is using new getBackgroun
         {
...
//               plane.setPixelColor(255, 100, 200);
            ...
            if ((red > 48 && green >= 0 && blue >= 0) && (red < 255 && green < 40 && blue < 40)) {
               plane.setPixelColor(0, 150, 130); //hat color
            }
            if ((red >= 0 && green >= 0 && blue >= 20) && (red < 10 && green < 10 && blue < 255)) {
               plane.setPixelColor(255, 255, 255); //stripe on hat
            }
         }
      }
      ...
      //eyes
      for (int row = 170; row < 260; row++)    //*** this is using new getBackground
      {
         for (int col = 300; col < 490; col++)    //*** this is using new getBackgroun
         {
            plane.teleport(col, row);
...
//               plane.setPixelColor(255, 100, 200);
            ...
            if ((red > 20 && green > 10 && blue >= 0) && (red < 255 && green < 255 && blue < 155)) {
               plane.setPixelColor(255, 0, 0); //hat color
            }
